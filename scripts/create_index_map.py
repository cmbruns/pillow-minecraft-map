...
def insert_index_map(map_name: str, max_palette: int = 144):
    bak = f"{map_name}.bak"
    if not os.path.exists(bak):
        shutil.copy2(map_name, bak)
    # Load a real Minecraft map info data into memory
    with gzip.open(map_name, mode="rb") as gz:
    ...
    x_center = int.from_bytes(bytes_blob[p_xc: p_xc + 4], byteorder="big", signed=True)
